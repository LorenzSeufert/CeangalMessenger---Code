...
class StepDefs : En {
    ...
    init {
        Given("I am on the main page") { throw PendingException() }

        Given(
            "I am signed in with username {string} and password {string}"
        ) { string: String?, string2: String? -> throw PendingException() }
        ...
        Then(
            "I get redirected to the {string} page"
        ) { string: String? -> throw PendingException() }
        ...
        Then(
            "I am on the {string} page again"
        ) { string: String? -> throw PendingException() }

        Given("the uploaded picture is not png, jpeg") { throw PendingException() }

        Then("a error message is shown") { throw PendingException() }
